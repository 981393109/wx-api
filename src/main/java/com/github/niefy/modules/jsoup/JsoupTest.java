package com.github.niefy.modules.jsoup;
...
//    private static String blogName = "guoxiaolongonly";
      private static String url = "http://news.baidu.com/house?loc=2354";
    ...
    public static void getArticleListFromUrl(final String listurl) {
        ...
        for (Element element : elements) {
            ...
            if (relHref.startsWith("http://") && relHref.contains("baijiahao.baidu.com/s?id=") ) {
...
//            String filename = elementTitle.text().replaceAll("/", "或");
            try {
                Element elementContent = document.getElementsByClass("article-title").first();//内容。
//            saveArticle(filename , elementContent.text(), blogName);
                System.out.println(elementContent.text());
                //得到第一张图片作为封面图
                Elements imgs = document.getElementsByTag("img");
                for (Element element : imgs) {
                    if(element.attr("src").contains("?token")){
